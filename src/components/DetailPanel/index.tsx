...
const DetailPanel = forwardRef<any, DetailProps>(
  ({ height, model, users, groups, messageDefs, signalDefs, onChange, readOnly = false }, ref) => {
    return (
      <div ref={ref} className={styles.detailPanel} style={{ height }}>
        ...
        {model.clazz === 'scriptTask' && (
          <ScriptTaskDetail model={model} onChange={onChange} readOnly={readOnly} />
        )}
        {model.clazz === 'javaTask' && (
          <JavaTaskDetail model={model} onChange={onChange} readOnly={readOnly} />
        )}
        {model.clazz === 'receiveTask' && (
          <ReceiveTaskDetail model={model} onChange={onChange} readOnly={readOnly} />
        )}
        {model.clazz === 'mailTask' && (
          <MailTaskDetail model={model} onChange={onChange} readOnly={readOnly} />
        )}
        ...
        {model.clazz === 'start' && (
          <StartEventDetail model={model} onChange={onChange} readOnly={readOnly} />
        )}
        {model.clazz === 'end' && (
          <EndEventDetail model={model} onChange={onChange} readOnly={readOnly} />
        )}
        {model.clazz === 'process' && (
